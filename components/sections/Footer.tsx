      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-fg font-display font-semibold text-sm">{siteConfig.name}</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <a href={siteConfig.social.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
              className="w-9 h-9 flex items-center justify-center rounded-lg text-fg-muted hover:text-accent hover:bg-accent/5 transition-all">
        ...
        {/* Bottom bar */}
        <div className="pt-6 border-t border-[rgba(108,99,255,0.06)] flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-fg-muted text-xs">&copy; {new Date().getFullYear()} {siteConfig.name}. All rights reserved.</span>
          <a href="#home" className="text-fg-muted text-xs hover:text-accent transition-colors">Back to top &uarr;</a>
        </div>
      </div>
    </footer>
  );
