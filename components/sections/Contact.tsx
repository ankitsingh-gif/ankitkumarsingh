  ...
  return (
    <section id="contact" className="section relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <motion.div className="flex flex-col sm:flex-row gap-4 justify-center mb-14" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}>
          <a href={`mailto:${siteConfig.email}`} className="btn-gradient inline-flex items-center justify-center gap-2.5" data-cursor="pointer">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            {siteConfig.email}
          </a>
          <a href={`tel:${siteConfig.phone}`} className="btn-outline inline-flex items-center justify-center gap-2.5" data-cursor="pointer">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <motion.div className="flex justify-center gap-3" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }}>
          {socialLinks.map((l) => (
            <a
              key={l.name}
              href={l.href}
              ...
              data-cursor="pointer"
              className="w-12 h-12 flex items-center justify-center rounded-xl border border-[rgba(108,99,255,0.12)] text-fg-secondary hover:text-accent hover:border-accent/30 hover:-translate-y-1 transition-all duration-300"
              aria-label={l.name}
            >
              {l.icon}
            </a>
