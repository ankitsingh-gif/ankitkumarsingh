...
export default function Navbar() {
  ...
  return (
    <>
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-10 transition-all duration-500 ${scrolled ? "py-3 bg-[#fafaf8]/90 backdrop-blur-xl border-b border-[rgba(108,99,255,0.08)]" : "py-5"}`}
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href="#contact" className="px-5 py-2 bg-accent/10 text-accent text-sm font-medium rounded-full border border-accent/30 hover:bg-accent hover:text-white transition-all duration-300">Let&apos;s Talk</a>
          </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div className="fixed inset-0 z-40 bg-[#fafaf8] pt-24 px-6 flex flex-col gap-4"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} onClick={() => setOpen(false)}
                className="text-[#1a1a1a] text-2xl font-display font-bold py-2 border-b border-[rgba(108,99,255,0.1)] hover:text-accent transition-colors">{link.label}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
